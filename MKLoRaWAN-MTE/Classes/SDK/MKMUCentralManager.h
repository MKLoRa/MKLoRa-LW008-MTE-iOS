...
extern NSString *const mk_mu_peripheralConnectStateChangedNotification;

//Notification of changes in the status of the Bluetooth Center.
extern NSString *const mk_mu_centralManagerStateChangedNotification;
 ...
 */
extern NSString *const mk_mu_deviceDisconnectTypeNotification;

@class CBCentralManager,CBPeripheral;
@class MKMUOperation;

@interface MKMUCentralManager : NSObject<MKBLEBaseCentralManagerProtocol>

@property (nonatomic, weak)id <mk_mu_centralManagerScanDelegate>delegate;

@property (nonatomic, weak)id <mk_mu_storageDataDelegate>dataDelegate;

@property (nonatomic, weak)id <mk_mu_centralManagerLogDelegate>logDelegate;

/// Current connection status
@property (nonatomic, assign, readonly)mk_mu_centralConnectStatus connectStatus;

+ (MKMUCentralManager *)shared;

/// Destroy the MKMUCentralManager singleton and the MKBLEBaseCentralManager singleton. After the dfu upgrade, you need to destroy these two and then reinitialize.
+ (void)sharedDealloc;

/// Destroy the MKMUCentralManager singleton and remove the manager list of MKBLEBaseCentralManager.
