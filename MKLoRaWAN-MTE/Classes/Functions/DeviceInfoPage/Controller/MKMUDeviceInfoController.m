...
@interface MKMUDeviceInfoController ()<UITableViewDelegate,
UITableViewDataSource,
MKMUTextButtonCellDelegate>
...
@property (nonatomic, strong)MKMUDeviceInfoModel *dataModel;
...
@implementation MKMUDeviceInfoController

- (void)dealloc {
    NSLog(@"MKMUDeviceInfoController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStartDFUProcess)
                                                 name:@"mk_mu_startDfuProcessNotification"
                                               object:nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 5 && indexPath.row == 0) {
        ...
        MKMUBatteryConsumptionController *vc = [[MKMUBatteryConsumptionController alloc] init];
    ...
    if (indexPath.section == 6 && indexPath.row == 0) {
        //Debugger Mode
        MKMUDebuggerController *vc = [[MKMUDebuggerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKMUTextButtonCell *cell = [MKMUTextButtonCell initCellWithTableView:tableView];
...
#pragma mark - MKMUTextButtonCellDelegate
/// 用户点击了右侧按钮
/// @param index cell所在序列号
- (void)mu_textButtonCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKMUUpdateController *vc = [[MKMUUpdateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)pushSelftestInterface {
    MKMUSelftestController *vc = [[MKMUSelftestController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)updateCellDatas {
    ...
    if (ValidStr(self.dataModel.firmware)) {
        MKMUTextButtonCellModel *firmwareModel = self.section1List[0];
...
- (void)loadSection1Datas {
    MKMUTextButtonCellModel *cellModel = [[MKMUTextButtonCellModel alloc] init];
    cellModel.index = 0;
...
- (MKMUDeviceInfoModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUDeviceInfoModel alloc] init];
    }
    return _dataModel;
}
