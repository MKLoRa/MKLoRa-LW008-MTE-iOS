...
@interface MKMUFilterByAdvNameModel ()
...
@implementation MKMUFilterByAdvNameModel
...
- (BOOL)readFilterPreciseMatch {
    ...
    [MKMUInterface mu_readFilterByAdvNamePreciseMatchWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByAdvNamePreciseMatch:self.match sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterByAdvNameReverseFilterWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByAdvNameReverseFilter:self.filter sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readAdvNameList {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterAdvNameListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvNameList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterAdvNameList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
