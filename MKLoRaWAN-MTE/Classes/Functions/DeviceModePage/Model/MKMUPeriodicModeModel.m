...
@interface MKMUPeriodicModeModel ()
...
@implementation MKMUPeriodicModeModel
...
- (BOOL)readStrategy {
    ...
    [MKMUInterface mu_readPeriodicModePositioningStrategyWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configStrategy {
    __block BOOL success = NO;
    [MKMUInterface mu_configPeriodicModePositioningStrategy:self.strategy sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReportInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_readPeriodicModeReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_configPeriodicModeReportInterval:[self.interval longLongValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
