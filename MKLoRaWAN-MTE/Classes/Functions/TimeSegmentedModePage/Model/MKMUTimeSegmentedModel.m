...
- (BOOL)configReportingTimePoint:(NSArray <MKMUTimeSegmentedTimePeriodModel *>*)pointList {
    __block BOOL success = NO;
    [MKMUInterface mu_configTimeSegmentedModeTimePeriodSetting:pointList sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
