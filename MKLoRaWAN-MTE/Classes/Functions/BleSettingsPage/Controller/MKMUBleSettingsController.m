...
@interface MKMUBleSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKMUBroadcastTxPowerCellDelegate>
...
@implementation MKMUBleSettingsController

- (void)dealloc {
    NSLog(@"MKMUBleSettingsController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKMUBroadcastTxPowerCell *cell = [MKMUBroadcastTxPowerCell initCellWithTableView:tableView];
...
#pragma mark - MKMUBroadcastTxPowerCellDelegate
/*
...
- (void)mu_txPowerValueChanged:(NSInteger)txPower {
    ...
    MKMUBroadcastTxPowerCellModel *cellModel = self.section1List[0];
...
- (void)configPassword{
    ...
    [alertView showAlertWithTitle:@"Change Password" message:msg notificationName:@"mk_mu_needDismissAlert"];
}
...
- (void)setPasswordToDevice{
    ...
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKMUInterface mu_configPassword:password sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKMUBroadcastTxPowerCellModel *cellModel = [[MKMUBroadcastTxPowerCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUBleSettingsController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUBleSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUBleSettingsModel alloc] init];
    }
    return _dataModel;
}
