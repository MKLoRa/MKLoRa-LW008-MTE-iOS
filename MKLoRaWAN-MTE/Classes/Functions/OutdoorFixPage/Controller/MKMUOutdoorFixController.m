...
@interface MKMUOutdoorFixController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUOutdoorFixModel *dataModel;

@end

@implementation MKMUOutdoorFixController

- (void)dealloc {
    NSLog(@"MKMUOutdoorFixController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUOutdoorFixController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUOutdoorFixModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUOutdoorFixModel alloc] init];
    }
    return _dataModel;
}
