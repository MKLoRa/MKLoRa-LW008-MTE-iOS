...
@interface MKMUManDownController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUManDownDataModel *dataModel;

@property (nonatomic, strong)MKAlertView *alertView;

@end

@implementation MKMUManDownController

- (void)dealloc {
    NSLog(@"MKMUManDownController销毁");
}
...
- (void)mu_textButtonCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        ...
        [self.alertView showAlertWithTitle:@"Reset Idle Status"
                          ...
                          notificationName:@"mk_mu_needDismissAlert"];
        return;
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUManDownController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUManDownDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUManDownDataModel alloc] init];
    }
    return _dataModel;
}
