...
@interface MKMUManDownDataModel ()
...
@implementation MKMUManDownDataModel
...
- (void)resetIdleStatusWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    [MKMUInterface mu_configIdleStutasResetWithSucBlock:sucBlock failedBlock:failedBlock];
}

#pragma mark - interface
- (BOOL)readManDownDetection {
    __block BOOL success = NO;
    [MKMUInterface mu_readManDownDetectionWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configManDownDetection {
    __block BOOL success = NO;
    [MKMUInterface mu_configManDownDetectionStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readIdleDetectionTimeout {
    __block BOOL success = NO;
    [MKMUInterface mu_readIdleDetectionTimeoutWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIdleDetectionTimeout {
    __block BOOL success = NO;
    [MKMUInterface mu_configIdleDetectionTimeout:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
