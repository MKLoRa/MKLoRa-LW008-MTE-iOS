...
@interface MKMUDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_mu_centralManagerLogDelegate,
MKMUDebuggerCellDelegate>
...
@property (nonatomic, strong)MKMUDebuggerButton *deleteButton;

@property (nonatomic, strong)MKMUDebuggerButton *exportButton;
...
@implementation MKMUDebuggerController

- (void)dealloc {
    NSLog(@"MKMUDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_mu_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_mu_startDebuggerMode" object:nil];
...
- (void)leftButtonMethod {
    if ([MKMUCentralManager shared].connectStatus != mk_mu_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
    ...
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKMUCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKMUDebuggerCell *cell = [MKMUDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - MKMUDebuggerCellDelegate
- (void)mu_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKMUDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_mu_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_export_disableIcon.png");
    ...
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_mu_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKMULogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKMUDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKMULogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKMUDebuggerCellModel *cellModel = [[MKMUDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKMULogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKMUDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_delete_disableIcon.png");
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_mu_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKMUDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKMUDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_delete_disableIcon.png");
...
- (MKMUDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKMUDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUDebuggerController", @"mu_export_disableIcon.png");
