//
//  MKMUBatteryConsumptionController.m
...
@interface MKMUBatteryConsumptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUBatteryConsumptionModel *dataModel;

@end

@implementation MKMUBatteryConsumptionController

- (void)dealloc {
    NSLog(@"MKMUBatteryConsumptionController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        MKMUBatteryInfoCell *cell = [MKMUBatteryInfoCell initCellWithTableView:tableView];
...
- (void)batteryReset {
    ...
    alertView.notificationName = @"mk_mu_needDismissAlert";
    @weakify(self);
...
- (void)sendBatteryResetCommandToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKMUInterface mu_batteryResetWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)loadSection0Datas {
    MKMUBatteryInfoCellModel *cellModel = [[MKMUBatteryInfoCellModel alloc] init];
...
- (MKMUBatteryConsumptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUBatteryConsumptionModel alloc] init];
    }
    return _dataModel;
}
