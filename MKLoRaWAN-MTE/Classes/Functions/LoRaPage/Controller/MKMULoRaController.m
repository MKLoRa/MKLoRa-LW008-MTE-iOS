...
@interface MKMULoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKMULoRaPageModel *dataModel;

@end

@implementation MKMULoRaController

- (void)dealloc {
    NSLog(@"MKMULoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKMULoRaSettingController *vc = [[MKMULoRaSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.row == 2) {
        //Application Settings
        MKMULoRaAppSettingController *vc = [[MKMULoRaAppSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (MKMULoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMULoRaPageModel alloc] init];
    }
    return _dataModel;
}
