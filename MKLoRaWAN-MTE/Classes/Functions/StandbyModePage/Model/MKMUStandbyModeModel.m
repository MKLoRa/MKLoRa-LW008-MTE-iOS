...
@interface MKMUStandbyModeModel ()
...
@implementation MKMUStandbyModeModel
...
- (BOOL)readStrategy {
    ...
    [MKMUInterface mu_readStandbyModePositioningStrategyWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configStrategy {
    __block BOOL success = NO;
    [MKMUInterface mu_configStandbyModePositioningStrategy:self.strategy sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
