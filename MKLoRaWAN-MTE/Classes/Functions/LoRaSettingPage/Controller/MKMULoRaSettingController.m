...
@interface MKMULoRaSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMULoRaSettingModel *dataModel;

@end

@implementation MKMULoRaSettingController

- (void)dealloc {
    NSLog(@"MKMULoRaSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMULoRaSettingController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMULoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMULoRaSettingModel alloc] init];
    }
    return _dataModel;
}
