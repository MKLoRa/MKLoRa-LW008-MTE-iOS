...
@interface MKMUFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUFilterByTofModel *dataModel;

@end

@implementation MKMUFilterByTofController

- (void)dealloc {
    NSLog(@"MKMUFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUFilterByTofController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUFilterByTofModel alloc] init];
    }
    return _dataModel;
}
