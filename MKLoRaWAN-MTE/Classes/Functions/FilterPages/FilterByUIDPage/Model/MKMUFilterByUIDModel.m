...
@interface MKMUFilterByUIDModel ()
...
@implementation MKMUFilterByUIDModel
...
- (BOOL)readFilterStatus {
    ...
    [MKMUInterface mu_readFilterByUIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByUIDStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNamespaceID {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterByUIDNamespaceIDWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNamespaceID {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByUIDNamespaceID:self.namespaceID sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readInstanceID {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterByUIDInstanceIDWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configInstanceID {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByUIDInstanceID:self.instanceID sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
