//
//  MKMUFilterByBXPTagController.m
...
@interface MKMUFilterByBXPTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUFilterByBXPTagModel *dataModel;

@end

@implementation MKMUFilterByBXPTagController

- (void)dealloc {
    NSLog(@"MKMUFilterByBXPTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUFilterByBXPTagController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUFilterByBXPTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUFilterByBXPTagModel alloc] init];
    }
    return _dataModel;
}
