...
@interface MKMUFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUFilterByTLMModel *dataModel;

@end

@implementation MKMUFilterByTLMController

- (void)dealloc {
    NSLog(@"MKMUFilterByTLMController销毁");
}
...
- (void)configFilterStatus:(BOOL)isOn {
    ...
    [MKMUInterface mu_configFilterByTLMStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configTLMVersion:(NSInteger)version {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKMUInterface mu_configFilterByTLMVersion:version sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKMUFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
