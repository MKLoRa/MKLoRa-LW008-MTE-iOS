//
//  MKMUFilterByAdvNameController.m
...
@interface MKMUFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUFilterByAdvNameModel *dataModel;

@end

@implementation MKMUFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKMUFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUFilterByAdvNameController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
