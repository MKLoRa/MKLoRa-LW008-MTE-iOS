...
@interface MKMUFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKMUFilterEditSectionHeaderViewDelegate,
MKMUFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKMUFilterByOtherModel *dataModel;

@end

@implementation MKMUFilterByOtherController

- (void)dealloc {
    NSLog(@"MKMUFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        ...
        MKMUFilterEditSectionHeaderViewModel *model = [[MKMUFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKMUFilterByRawDataCell *cell = [MKMUFilterByRawDataCell initCellWithTableView:tableView];
...
- (void)mk_mu_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKMUFilterByRawDataCellModel *cellModel = [[MKMUFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKMUFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)mu_rawFilterDataChanged:(mk_mu_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKMUFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_mu_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_mu_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_mu_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_mu_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKMUFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKMUFilterRawAdvDataModel *model = [[MKMUFilterRawAdvDataModel alloc] init];
...
- (mk_mu_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_mu_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_mu_filterByOther_AB;
        }
        return mk_mu_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_mu_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_mu_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_mu_filterByOther_AOrBOrC;
        }
    }
    return mk_mu_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUFilterByOtherController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
