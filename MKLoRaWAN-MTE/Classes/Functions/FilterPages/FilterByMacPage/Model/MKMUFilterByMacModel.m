...
@interface MKMUFilterByMacModel ()
...
@implementation MKMUFilterByMacModel
...
- (BOOL)readFilterPreciseMatch {
    ...
    [MKMUInterface mu_readFilterByMacPreciseMatchWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByMacPreciseMatch:self.match sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterByMacReverseFilterWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByMacReverseFilter:self.filter sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacList {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterMACAddressList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
