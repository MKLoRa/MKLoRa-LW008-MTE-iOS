...
@interface MKMUFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUFilterByMacModel *dataModel;

@end

@implementation MKMUFilterByMacController

- (void)dealloc {
    NSLog(@"MKMUFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUFilterByMacController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUFilterByMacModel alloc] init];
    }
    return _dataModel;
}
