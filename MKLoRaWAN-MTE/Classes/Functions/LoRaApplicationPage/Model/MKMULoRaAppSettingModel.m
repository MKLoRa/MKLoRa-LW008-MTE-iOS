...
@interface MKMULoRaAppSettingModel ()
...
@implementation MKMULoRaAppSettingModel
...
- (BOOL)readTimeSyncInterval {
    ...
    [MKMUInterface mu_readLorawanTimeSyncIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeSyncInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_configTimeSyncInterval:[self.timeSyncInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNetworkCheckInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_readLorawanNetworkCheckIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkCheckInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_configLorawanNetworkCheckInterval:[self.checkInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
