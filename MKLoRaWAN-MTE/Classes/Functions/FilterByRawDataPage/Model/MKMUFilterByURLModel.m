...
@interface MKMUFilterByURLModel ()
...
@implementation MKMUFilterByURLModel
...
- (BOOL)readFilterStatus {
    ...
    [MKMUInterface mu_readFilterByURLStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByURLStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readURLContent {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterByURLContentWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configURLContent {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByURLContent:self.url sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
