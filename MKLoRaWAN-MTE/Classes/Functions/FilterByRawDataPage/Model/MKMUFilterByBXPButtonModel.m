...
@interface MKMUFilterByBXPButtonModel ()
...
@implementation MKMUFilterByBXPButtonModel
...
- (BOOL)readFilterStatus {
    ...
    [MKMUInterface mu_readBXPButtonFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByBXPButtonStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readButtonAlarmContent {
    __block BOOL success = NO;
    [MKMUInterface mu_readBXPButtonAlarmFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBXPButtonContent {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByBXPButtonAlarmStatus:self.singlePress
