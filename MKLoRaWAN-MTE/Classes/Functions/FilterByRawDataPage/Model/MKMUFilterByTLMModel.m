...
@interface MKMUFilterByTLMModel ()
...
@implementation MKMUFilterByTLMModel
...
- (BOOL)readFilterStatus {
    ...
    [MKMUInterface mu_readFilterByTLMStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByTLMStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTLMVersion {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterByTLMVersionWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTLMVersion {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByTLMVersion:self.tlm sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
