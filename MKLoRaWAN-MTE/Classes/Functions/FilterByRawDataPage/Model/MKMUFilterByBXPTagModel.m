...
@interface MKMUFilterByBXPTagModel ()
...
@implementation MKMUFilterByBXPTagModel
...
- (BOOL)readFilterStatus {
    ...
    [MKMUInterface mu_readFilterByBXPTagIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterByBXPTagIDStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readFilterPreciseMatch {
    __block BOOL success = NO;
    [MKMUInterface mu_readPreciseMatchTagIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKMUInterface mu_configPreciseMatchTagIDStatus:self.precise sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKMUInterface mu_readReverseFilterTagIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKMUInterface mu_configReverseFilterTagIDStatus:self.reverse sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTagIDList {
    __block BOOL success = NO;
    [MKMUInterface mu_readFilterBXPTagIDListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTagIDList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKMUInterface mu_configFilterBXPTagIDList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
