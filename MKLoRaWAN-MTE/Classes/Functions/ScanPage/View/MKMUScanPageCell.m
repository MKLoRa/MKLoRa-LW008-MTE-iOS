...
@implementation MKMUScanPageCell

+ (MKMUScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKMUScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMUScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKMUScanPageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMUScanPageCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(mu_scanCellConnectButtonPressed:)]) {
        [self.delegate mu_scanCellConnectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKMUScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMUScanPageModel.class]) {
        return;
    }
    //顶部
...
- (UIImageView *)rssiIcon {
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUScanPageCell", @"mu_scan_rssiIcon.png");
    }
    return _rssiIcon;
}
...
- (UIImageView *)batteryIcon {
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUScanPageCell", @"mu_scan_batteryIcon.png");
    }
    return _batteryIcon;
}
