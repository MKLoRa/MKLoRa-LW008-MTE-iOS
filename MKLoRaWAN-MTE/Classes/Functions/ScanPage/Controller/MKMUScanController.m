...
static NSString *const localPasswordKey = @"mk_mu_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKMUScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_mu_centralManagerScanDelegate,
MKMUScanPageCellDelegate,
MKSearchButtonDelegate,
MKMUTabBarControllerDelegate>
...
@implementation MKMUScanController

- (void)dealloc {
    NSLog(@"MKMUScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKMUCentralManager shared] stopScan];
    [MKMUCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKMUCentralManager shared].delegate = self;
...
- (void)rightButtonMethod {
    UIViewController *vc = [[CTMediator sharedInstance] CTMediator_LORAWAN_MU_AboutPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKMUScanPageCell *cell = [MKMUScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_mu_centralManagerScanDelegate
- (void)mk_mu_receiveDevice:(NSDictionary *)deviceModel {
    MKMUScanPageModel *model = [MKMUScanPageModel mk_modelWithJSON:deviceModel];
...
#pragma mark - MKMUScanPageCellDelegate
- (void)mu_scanCellConnectButtonPressed:(NSInteger)index {
    [self connectDeviceWithModel:self.dataList[index]];
}

#pragma mark - MKMUTabBarControllerDelegate
- (void)mk_mu_needResetScanDelegate:(BOOL)need {
    if (need) {
        [MKMUCentralManager shared].delegate = self;
    }
...
- (void)scanTimerRun{
    ...
    [[MKMUCentralManager shared] startScanWithDeviceType:self.deviceType];
...
- (void)updateDataWithScanModel:(MKMUScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKMUScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKMUScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKMUScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKMUScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKMUScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKMUScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKMUScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKMUCentralManager shared] stopScan];
    if (self.scanTimer) {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:msg notificationName:@"mk_mu_needDismissAlert"];
}

- (void)connectDeviceWithDataModel:(MKMUScanPageModel *)scanDataModel {
    ...
    [[MKMUConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? password : @"") deviceType:scanDataModel.deviceType sucBlock:^{
...
- (void)configParams {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"mu_readRecordDataDayNumKey"];
    //读取设备存储的扫描信息页面，显示的设备总共存储了多少条的扫描数据
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"mu_recordDataTotalSumKey"];
    
    [MKMUDatabaseManager clearDataTable];
    [MKMUDatabaseManager initDataBase];
}

- (void)pushTabBarPage {
    MKMUTabBarController *vc = [[MKMUTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUScanController", @"mu_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-MTE", @"MKMUScanController", @"mu_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
