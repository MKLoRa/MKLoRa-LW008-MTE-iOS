...
@interface MKMUTimingModeController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKMUTimingModeAddCellDelegate,
MKMUReportTimePointCellDelegate>
...
@property (nonatomic, strong)MKMUTimingModeModel *dataModel;

@end

@implementation MKMUTimingModeController

- (void)dealloc {
    NSLog(@"MKMUTimingModeController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKMUTimingModeAddCell *cell = [MKMUTimingModeAddCell initCellWithTableView:tableView];
        cell.dataModel = self.section1List[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    MKMUReportTimePointCell *cell = [MKMUReportTimePointCell initCellWithTableView:tableView];
...
#pragma mark - MKMUTimingModeAddCellDelegate
...
#pragma mark - MKMUReportTimePointCellDelegate
/**
 删除
...
- (BOOL)cellCanSelected{
    ...
    for (int i = 0; i < [arrCells count]; i++) {
        ...
        if ([cell isKindOfClass:MKMUReportTimePointCell.class]) {
            MKMUReportTimePointCell *tempCell = (MKMUReportTimePointCell *)cell;
...
- (void)loadSection2Datas {
    for (NSInteger i = 0; i < self.dataModel.pointList.count; i ++) {
        MKMUTimingModeTimePointModel *tempModel = self.dataModel.pointList[i];
        MKMUReportTimePointCellModel *cellModel = [[MKMUReportTimePointCellModel alloc] init];
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(i + 1)];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUTimingModeController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUTimingModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUTimingModeModel alloc] init];
    }
    return _dataModel;
}
