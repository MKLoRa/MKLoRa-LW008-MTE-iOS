...
@interface MKMUDeviceSettingModel ()
...
@implementation MKMUDeviceSettingModel
...
- (BOOL)readTimeZone {
    ...
    [MKMUInterface mu_readTimeZoneWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeZone {
    __block BOOL success = NO;
    [MKMUInterface mu_configTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPayload {
    __block BOOL success = NO;
    [MKMUInterface mu_readLowPowerPayloadStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPayload {
    __block BOOL success = NO;
    [MKMUInterface mu_configLowPowerPayloadStatus:self.lowPowerPayload sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPayloadInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_readLowPowerPayloadIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPayloadInterval {
    __block BOOL success = NO;
    [MKMUInterface mu_configLowPowerPayloadInterval:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPrompt {
    __block BOOL success = NO;
    [MKMUInterface mu_readLowPowerPromptWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPrompt {
    __block BOOL success = NO;
    [MKMUInterface mu_configLowPowerPrompt:self.prompt sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
