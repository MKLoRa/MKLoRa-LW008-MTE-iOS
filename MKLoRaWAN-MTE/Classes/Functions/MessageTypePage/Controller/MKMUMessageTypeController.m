...
@interface MKMUMessageTypeController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMUMessageTypeModel *dataModel;

@end

@implementation MKMUMessageTypeController

- (void)dealloc {
    NSLog(@"MKMUMessageTypeController销毁");
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) {
        return (self.dataModel.positionType == 1 ? self.section1List.count : 1);
    }
    if (section == 2) {
    ...
    if (section == 5) {
        return (self.dataModel.eventType == 1 ? self.section5List.count : 1);
    }
    if (section == 6) {
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MTE", @"MKMUMessageTypeController", @"mu_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMUMessageTypeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMUMessageTypeModel alloc] init];
    }
    return _dataModel;
}
