...
@implementation MKMUFilterBeaconCellModel
@end

@interface MKMUFilterBeaconCell ()
...
@implementation MKMUFilterBeaconCell

+ (MKMUFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKMUFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMUFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKMUFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMUFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKMUFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMUFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_mu_beaconMinValueChanged:index:)]) {
                [self.delegate mk_mu_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_mu_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_mu_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
