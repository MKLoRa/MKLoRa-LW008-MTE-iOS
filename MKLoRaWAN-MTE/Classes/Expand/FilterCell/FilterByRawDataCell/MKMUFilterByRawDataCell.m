...
@implementation MKMUFilterByRawDataCellModel
...
@interface MKMUFilterByRawDataCell ()
...
@implementation MKMUFilterByRawDataCell

+ (MKMUFilterByRawDataCell *)initCellWithTableView:(UITableView *)tableView {
    MKMUFilterByRawDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMUFilterByRawDataCellIdenty"];
    if (!cell) {
        cell = [[MKMUFilterByRawDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMUFilterByRawDataCellIdenty"];
    }
    return cell;
}
...
- (void)textFieldValueChanged:(NSString *)text textType:(mk_mu_filterRawAdvDataTextType)type {
    if ([self.delegate respondsToSelector:@selector(mu_rawFilterDataChanged:index:textValue:)]) {
        [self.delegate mu_rawFilterDataChanged:type index:self.dataModel.index textValue:text];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKMUFilterByRawDataCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMUFilterByRawDataCellModel.class]) {
        return;
    }
...
- (MKTextField *)typeTextField {
    if (!_typeTextField) {
        ...
        _typeTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [sself textFieldValueChanged:text textType:mk_mu_filterRawAdvDataTextTypeDataType];
        };
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_mu_filterRawAdvDataTextTypeMinIndex];
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_mu_filterRawAdvDataTextTypeMaxIndex];
        };
    }
...
- (MKTextField *)rawDataField {
    if (!_rawDataField) {
        ...
        _rawDataField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_mu_filterRawAdvDataTextTypeRawDataType];
        };
    }
