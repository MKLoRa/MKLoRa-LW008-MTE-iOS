...
@implementation MKMUTextButtonCellModel
@end

@interface MKMUTextButtonCell ()
...
@implementation MKMUTextButtonCell

+ (MKMUTextButtonCell *)initCellWithTableView:(UITableView *)tableView {
    MKMUTextButtonCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMUTextButtonCellIdenty"];
    if (!cell) {
        cell = [[MKMUTextButtonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMUTextButtonCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(mu_textButtonCell_buttonAction:)]) {
        [self.delegate mu_textButtonCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKMUTextButtonCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMUTextButtonCellModel.class]) {
        return;
    }
